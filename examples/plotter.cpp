...
void Plotter::plotAnimationFramePython(int frame_nb){
    ...
    std::ofstream file1(output_folder_ + "circles.csv");
    if (file1.is_open()){
        for (int i = 0; i < circle_x_.size(); i++){
            file1 <<"circle "<<i;
    ...
    std::ofstream file2(output_folder_ + "corridors.csv");
    if (file2.is_open()){
    ...
    if (adaptive_solution_available_){
        std::ofstream file3(output_folder_ + "results_adaptive.csv");
        if (file3.is_open()){
        ...
        std::ofstream file4(output_folder_ + "solutions_adaptive.csv");
    ...
    if (ref_solution_available_){
        std::ofstream file5(output_folder_ + "results_ref.csv");
        if (file5.is_open()){
        ...
        std::ofstream file6(output_folder_ + "solutions_ref.csv");
    ...
    if (naive_solution_available_){
        std::ofstream file7(output_folder_ + "results_naive.csv");
        if (file7.is_open()){
        ...
        std::ofstream file8(output_folder_ + "solutions_naive.csv");
...
void Plotter::plotComputationTimesFullFull(){
    ...
    for (int plot = 0; plot < timings.size()/3; plot++){
        std::ofstream file(output_folder_ + "timings_" + std::to_string(plot) + 
                           ".csv");
        if (file.is_open()){
...
void Plotter::writeScaling(){
    ...
    std::ofstream file(output_folder_ + "scaling_data.csv");
    if (file.is_open()){

#include "../include/adaptiveCorridorExampleHelpers.hpp"
...
bool AdaptiveCorridorHelper::processObstacleConstraints(
    ...
    if (std::pow(x0[0]-pos_x, 2) + std::pow(x0[1] - pos_y, 2) <= 
        ...
        std::vector<int> k_vals = getCollidingPoints(pos_x, pos_y, r, sol);
        nlp_->addExtraConstraint(k_vals, {c_idx}, {instance_idx}, 
                                 {{pos_x, pos_y, r}});
        return true;
    }
    return false;
...
std::vector<std::vector<double>> AdaptiveCorridorHelper::performAdaptiveLoop(
    ...
    ///////////////////////////////////////////
    // do the loop using the Adaptive Method //
    ///////////////////////////////////////////
...
std::vector<std::vector<double>> AdaptiveCorridorHelper::performCasadiLoop(
    ...
    std::vector<double> init_guess;
    bool v;
    std::vector<std::vector<double>> detailed_timings(6, 
        std::vector<double>(nb_iterations, 0.0));
